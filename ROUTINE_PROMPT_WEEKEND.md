...
3. **리서치**: WebSearch로 조사한다. 지난 한 주의 평일 콘텐츠 파일들(`content/` 의 이번 주 am/pm JSON)을 먼저 읽으면 한 주 흐름을 빠르게 파악할 수 있다 — 단, 주간 등락률·최신 일정은 반드시 웹에서 다시 확인한다.
   ...
   **`hook_bull`·`hook_bear` 도 같은 프롬프트에 함께 넘겨 검증한다** — 첫 카드에 나가는 문구라 오류 시
   임팩트가 가장 크다. 단 검증 범위는 세션에 따라 다르다:
   - `sat`: 인용한 **사실(수치·날짜·사건)** 이 맞는지. 인과 해석의 타당성은 판정시키지 않는다.
   - `sun`: ⓐ **그 일정이 실제로 그 날짜에 예정돼 있는지**(공식 발표 일정·기업 IR 공지 기준), ⓑ 인용한
     **직전 수치·컨센서스·가이던스가 맞는지** 두 가지만 본다. "둔화될 것인가" 같은 **전망 자체의 타당성은
     검증 대상이 아니다** — 정답이 없는 판정을 시키면 검증 결과 전체가 흐려진다.
...
4. **콘텐츠 JSON 작성**: `content/<DATE>-<SESSION>.json` 으로 저장한다. 스키마는 평일(ROUTINE_PROMPT.md 4단계)과 동일하며, 아래 주말 전용 값을 쓴다.
   ...
   - **`hook_bull` / `hook_bear`**: 1번 훅 카드의 초록 위쪽 블록 · 빨강 아래쪽 블록.
     평일 절차서 3-b-2 의 작성 원칙(구체적 종목·수치, headline 과 비중복, 각 1건씩)을 따르되,
     주말은 범위가 한 주이고 **세션에 따라 성격이 완전히 다르다**:

     - **`sat` — 지나간 한 주 (라벨: "▲ 호재" / "▼ 악재")**
       이번 주를 통틀어 **가장 긍정적이었던 것 / 가장 부담이었던 것** 한 문장씩.
       이미 일어난 일이므로 평일과 동일하게 **확실한 사실만** 쓴다.

     - **`sun` — 다음 주 예정 (라벨: "▲ 기대 요인" / "▼ 경계 요인")**
       아직 일어나지 않은 일을 다루므로 "확실한 사실"이라는 기준을 그대로 적용할 수 없다.
       대신 아래 세 가지를 지킨다:
       1. **일정 자체가 확정된 것만 쓴다** — 발표일이 공시된 지표, 확정된 실적 발표일.
          "~할 것으로 보인다" 수준의 미확정 이벤트는 넣지 않는다.
       2. **기대·경계의 근거가 구체적이어야 한다** — 컨센서스 전망치, 직전 발표 수치,
          회사 가이던스 등. "시장이 기대하고 있습니다" 같은 근거 없는 서술은 쓰지 않는다.
       3. **단정적 예측을 하지 않는다.** 결과를 미리 규정하지 말고 무엇이 관건인지를 쓴다.
          - ✕ "수요일 CPI가 둔화되며 금리 인하 기대가 커질 것입니다"
          - ○ "수요일 CPI — 직전 3.1%에서 둔화되는지가 이번 주 금리 기대의 분기점입니다"
       라벨이 "호재/악재"가 아니라 "기대 요인/경계 요인"으로 자동 전환되므로(렌더러가 세션으로 판단),
       문장도 그 톤에 맞춘다.
