...
- **카드3 의 `flows.rows` 는 최대 3행이다.** `breadth`(2행) 아래 `flows` 가 이어 그려지는데, 그 아래 `note` 블록은 카드 안에서 고정된 위치(`top:834px`)에 있다 — `flows.rows` 가 4행(예: 개인·외국인·기관·기타법인)이 되면 마지막 행이 note 와 겹친다(2026-08-25 pm 세션에서 실측). `data/krx-flows.json` 은 기타법인(otherCorp)까지 네 주체를 다 주지만, 카드에는 개인·외국인·기관 3행만 넣고 기타법인은 `note_ko/en` 문장으로 풀어 쓴다.
