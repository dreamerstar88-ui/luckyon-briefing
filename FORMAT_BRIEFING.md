...
- **카드3 에서 `breadth`·`flows` 를 둘 다 생략하면 "등락 종목수" 소제목 자체가 안 그려진다** (2026-09-24 수정). 한국 시장이 휴장이라 record 카드를 미국 선물/아시아 지수 등으로 대체하는 회차처럼 등락 종목수 데이터가 아예 없을 때, 예전에는 소제목만 찍히고 그 아래가 빈 채로 남았다(2026-09-24 pm 세션에서 실측) — 지금은 둘 다 없으면 그 블록 전체가 생략된다. `breadth`·`flows` 중 하나라도 있으면 예전처럼 그려진다.
- **카드3 의 `flows.rows` 는 최대 3행이다.** `breadth`(2행) 아래 `flows` 가 이어 그려지는데, 그 아래 `note` 블록은 카드 안에서 고정된 위치(`top:834px`, `bottom:132px`)에 있다 — `flows.rows` 가 4행(예: 개인·외국인·기관·기타법인)이 되면 마지막 행이 note 와 겹친다(2026-08-25 pm 세션에서 실측). `data/krx-flows.json` 은 기타법인(otherCorp)까지 네 주체를 다 주지만, 카드에는 개인·외국인·기관 3행만 넣고 기타법인은 `note_ko/en` 문장으로 풀어 쓴다.
  - **3행이어도 겹칠 수 있다 — `flows.unit_ko/en` 을 채우면 특히 그렇다.** `note` 는 `top`이 아니라 카드 바닥에서 `bottom:132px` 고정이라, 그 위 `breadth`+`flows`(라벨 줄+3행+`unit` 줄)가 총 몇 줄인지와 무관하게 자리를 침범할 수 있다. `flows.unit_*` 을 채우면 행 하나만큼의 높이가 더 필요해 국문·영문 모두에서 겹침이 생길 수 있다(2026-09-01 pm 세션에서 ko·en 카드 모두 "기관" 행과 note 가 겹치는 것을 렌더 확인 단계에서 잡았다). **`unit`은 별도 필드에 채우지 말고 `flows.label_ko/en` 안에 괄호로 접어 넣는다** (예: `"코스피 투자자별 순매수 (억원)"` / `"KOSPI net buy/sell by investor (₩100M)"`) — 그러면 `unit` 줄 자체가 안 그려져 한 줄만큼 여유가 생긴다. 그래도 영문에서 헤드라인·차트 부제·`note` 가 길어 줄바꿈이 늘면 다시 겹칠 수 있으니, 이 카드의 `note_en`은 1줄(약 140자 이내)로 짧게 쓴다.
  - **`flows.rows[].value` 는 순수 숫자(number)여야 한다 — 문자열 금지.** `"−30,332"`처럼 콤마·부호가 든 문자열을 넣으면 렌더러(`cardkit.mjs`의 `flowRows`)가 `r.value.toFixed(2)`를 호출하다가 `TypeError: r.value.toFixed is not a function`로 렌더 자체가 죽는다(2026-09-08 pm 세션에서 실제로 발생). `-30332`처럼 부호 있는 순수 숫자로 넣는다 — 렌더러가 부호·막대 폭을 알아서 계산한다. 단위(억원 등)는 위 항목대로 `flows.label_ko/en` 괄호에 적는다.
- **`rank` 의 `rows[].pct` 는 "그 종목 자체의 등락률"이지, 상위 종목이 전체에서 차지하는 비중이 아니다.** 비중(거래대금 집중도 등)은 별도 필드인 `share.segments[].pct` 로 표현한다. 둘을 헷갈려 `rows[].pct` 에 비중을 넣으면 순위표 옆 등락 표시가 그 종목의 실제 등락과 달라진다.
  - **`rows[].pct` 도 순수 숫자(number)다 — `"+0.56%"` 같은 문자열을 넣으면 등락률 칸이 `-NaN%`으로 깨진다.** 렌더러가 `pctColor(r.pct)`·`fmtPct(r.pct)`로 직접 연산한다(2026-09-08 pm 세션 실측). `0.56`·`-0.19`처럼 부호 있는 숫자만 넣는다 — 기호(`▲`/`▼`)와 `%`는 렌더러가 붙인다.
- **`rows[].value` 는 언어 무관(invariant) 필드다** (`FORMAT_BRIEFING.md` §4). 원화 단위를 넣을 때 `"9.11조"` 처럼 한글을 넣으면 영어 카드에도 그대로 새어 나간다 — `"₩9.11T"` 처럼 중립 표기를 쓴다.
  - **막대 폭은 `value` 문자열에서 숫자만 추출해 정하므로, 같은 카드 안의 모든 행은 단위를 통일해야 한다.** 렌더러가 `String(r.value).replace(/[^0-9.]/g, '')`로 접두 기호·단위 문자를 전부 지우고 남은 숫자만 비교한다 — `"₩4.03T"`(=4.03)와 `"₩361.9B"`(=361.9)를 한 카드에 같이 쓰면 실제로는 4.03조가 3619억보다 훨씬 큰데도 361.9가 더 큰 숫자로 읽혀 거래대금이 가장 작은 행의 막대가 가장 길게 그려진다(2026-09-08 pm 세션에서 실제로 발생 — 4.03T·3.25T·361.9B를 섞어 361.9B 행만 막대가 꽉 찼다). **한 카드의 `rows[].value`는 전부 같은 단위(전부 T, 또는 전부 B)로 맞춘다** — 예: `"₩0.36T"`처럼 소수로 내려서라도 단위를 통일한다.
...
- **`share.segments[].color` 는 반드시 헥스 코드로 쓴다 (`"#3987e5"` 처럼) — `"amber"`·`"gray"` 같은 색상 이름을 쓰지 않는다.** 렌더러(`render-cards-day.mjs`)가 이 값을 검증 없이 그대로 CSS `background:`에 꽂는다. `gray`처럼 우연히 유효한 CSS 키워드도 있지만 `amber`는 CSS 표준 색상 이름이 아니라서 조용히 무시되고, 그 구간만 색이 안 칠해진 채(배경이 그대로 비쳐) 렌더된다 — 오류 로그도 없이 카드만 밋밋하게 나온다(2026-08-27 am 세션에서 실제로 발생, `share.mode:"nested"` 막대의 작은 구간이 색 없이 나갔다). 이 결함은 **렌더링 이후 카드 이미지를 직접 열어봐야만** 보이고 검증 에이전트의 JSON 검토로는 잡을 수 없으므로(§3 순서상 검증은 렌더링 전에 끝난다), 카드 분량 확인(§6) 때 텍스트 겹침뿐 아니라 **막대·조각마다 실제로 서로 다른 색이 칠해졌는지도 함께 눈으로 확인한다.**
...
- **`stats[].ref_ko`/`ref_en` — 그 지표가 «무엇을 뜻하는 숫자인지» 한 줄** (2026-08-26 신설, 카드6).
  타일 바닥에 구분선과 함께 작게 붙는다.
  - **보통은 안 써도 된다 — 렌더러가 `data/stat-refs.json` 에서 찾아 자동으로 채운다.** 라벨에서
    지표를 알아보고(`소비자신뢰지수` → `1985년 = 100 기준 …`) 넣어 주므로, 자주 나오는 지표는
    손댈 일이 없다. 렌더 로그에 `카드6 기준문구 자동 채움 N건` 으로 무엇이 채워졌는지 찍힌다.
  - **직접 쓰면 그쪽이 이긴다.** 사전에 없는 지표이거나 그 회차에만 맞는 설명이 필요할 때만 쓴다.
  - **사전에 없으면 그 줄만 비고 카드는 정상이다.** 자주 나올 지표라면 `data/stat-refs.json` 에
    항목을 추가한다(그래야 다음 회차부터 공짜로 붙는다). 점검: `node scripts/stat-ref.mjs --check <DATE> <SESSION>`
...
- **`valuation` 은 `data/index-valuation.json` 에서 복사하고 `note` 에 출처를 반드시 적는다.**
  ...
  **EPS 는 쓰지 않는다**(소스에 없고 역산하면 오차가 값처럼 보인다). 현재 PER 옆에 1년 전 대비가
  ...
  - **`valuation.note_ko`/`note_en` 만은 `※` 로 시작하지 않는다.** 렌더러가 이 한 필드에만 `※` 를
    직접 앞에 붙여 그린다(`render-cards-sat.mjs` 의 valuation 블록). 문구에도 `※` 를 넣으면
    카드에 **`※ ※` 로 두 번** 나온다 (2026-08-29 sat 에서 실제로 그렇게 렌더돼 재렌더했다).
    반대로 `metricsNote_*`·`sectorsNote_*`·`movers.note_*`·`indexNote_*` 는 공용 `note()` 가
    그대로 출력하므로 **문구에 `※` 를 직접 넣어야** 한다 — 필드마다 규칙이 반대다.
  - **`data/index-valuation.json` 이 낡았으면 그 기준일을 `note` 에 박아 싣는다.** 세션은 WSJ 에
    403 이라 직접 받을 수 없고(설계대로다), `index-valuation` 워크플로가 그 주에 실패하면 지난주
    값이 남아 있다(2026-08-29 sat 실측 — asOf 8/21). 일주일 전 PER 은 «틀린 숫자»가 아니라
    «기준일이 다른 숫자»이므로, 기준일을 밝히면 빈칸보다 낫다. 밝히지 않고 실으면 그때는
    빈칸이 낫다.
...
| 카드 | 증상 | 대처 |
|---|---|---|
| 글 카드(`items`) | 6건 꽉 채우면(카드7·8) 마지막 항목이 통째로 잘려 아예 안 보이거나(푸터도 사라짐) 마지막 항목 본문이 중간에 끊긴다 — 4~5건일 때는 출처 줄만 푸터에 닿는 정도지만 6건이면 훨씬 치명적이다(2026-09-01 pm 세션에서 en 카드가 실제로 6번째 항목째 통째로 안 보였다). `body_en` 이 160자를 넘거나 `headline_en` 이 길어 2줄로 접히는 항목이 하나라도 있으면 6건일 때 넘치기 쉽다 | 항목 6건일 땐 특히 **`headline_en` 을 1줄에 들어가게(대략 55자 안쪽) 줄이고 `body_en` 을 140~150자 안쪽으로** 빡빡하게 쓴다(문서 상한 160자는 여유가 아니라 상한이다). 렌더 후 반드시 마지막 카드까지 이미지를 열어 푸터·페이지번호가 보이는지 확인한다 |
