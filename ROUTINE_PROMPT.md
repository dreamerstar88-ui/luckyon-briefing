   ...
   **3-a. 시장 지표 (markets 10개 타일)** — **아침·저녁 동일한 10개**로 통일한다 (순서 고정):
   ...
   - **세션별로 다른 것은 "타일 종류"가 아니라 "값의 성격"뿐이며, 이는 한 줄 해석(note)으로 밝힌다:**
     ...
     - `pm`(저녁, 미국장 개장 전): S&P 500·나스닥은 **선물(futures)** 로 표기하고 note 에 "선물"임을 밝힌다. 값은 `node scripts/fetch-futures.mjs` 로 조회한다 (Apify 액터 사용, `apify/futures-scraper/` 참고). 코스피는 당일 종가.
       - **선물 조회 실패·건너뜀 시 대체 (중요)**: `fetch-futures.mjs` 가 exit 2(APIFY_TOKEN 미설정) 또는 오류로 값을 못 주면, **발행을 멈추지 말고** S&P 500·나스닥을 직전 미국장 **종가**로 표기하고 note 에 "전일 종가·개장 전"이라고 밝힌다. (Apify 토큰이 준비되면 자동으로 선물 값으로 복귀한다.)
