...
3. **뉴스 리서치**: WebSearch로 최신 뉴스를 조사한다. (2-a에서 기존 파일을 재사용하기로 했다면 이 단계는 건너뛴다.)
   ...
   **3-a. 시장 지표 (markets 10개 타일)** — 세션별로 구성이 다르다:
   ...
   - **휴장 처리 (중요)**: 한국·미국 등 해당 시장이 공휴일·주말로 휴장이어도 **타일을 절대 빼지 않는다.** 타일 개수·배치를 항상 10개로 일정하게 유지한다. 휴장 타일은 `value` 에 직전 종가(또는 마지막 고시가)를 그대로 두고, `delta` 는 `"휴장"`, `dir` 은 `"flat"`, 한 줄 해석(`note_ko`/`note_en`)에 사유를 적는다 (예: `"서울 제헌절 휴장, 원화 고시 없음"` / `"No onshore fixing — Seoul holiday"`). 지수 선물처럼 휴장 중에도 거래되는 항목은 정상 표기한다.
