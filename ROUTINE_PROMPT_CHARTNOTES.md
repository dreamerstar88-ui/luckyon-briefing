...
그 이슈는 **승인 확인 루틴**(`ROUTINE_PROMPT_CHARTNOTES_APPROVE.md` — GitHub 이벤트 트리거로 **승인 즉시** 뜬다)이 읽어 처리한다.
...
발행이 시작되는 경로는 두 가지다. **어느 쪽이든 아래 P1~P4 는 동일하다.**

1. **GitHub 이슈** — 사용자가 검토 페이지의 `✅ 발행 승인` 버튼으로 이슈를 남기면,
   승인 확인 루틴(`ROUTINE_PROMPT_CHARTNOTES_APPROVE.md`)이 그것을 읽고 여기를 수행한다.
   사용자가 밖에 있어도 폰만으로 되는 경로다.
2. **직접 지시** — 사용자가 Claude 에게 `차트노트 <STAMP> 발행해` 라고 하는 경우.
