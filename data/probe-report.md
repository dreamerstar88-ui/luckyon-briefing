...
사용자가 PlayMCP 커넥터를 계정 설정에서 켰으나, 이 세션(예약 실행 세션)에는 도구가 로드되지 않음(`enabledInChat: false`, ToolSearch/SearchMcpRegistry 재검색해도 PlayMCP 관련 도구 미검출). 예약 세션은 시작 시점에 연결된 커넥터 목록이 고정되는 것으로 추정됨. **PlayMCP를 통한 KOSPI·KOSDAQ·지수 선물 보충은 새 대화(세션)에서 별도로 점검 필요** — 이 리포트에는 미포함.

## 결론 (최신)

무료 티어로 아래 14개 세부 항목(브리핑 카드② 타일 관련) 중 **10개 확보 가능** (미국 3대 지수, 닛케이, VIX, 10년물 금리, 원/달러, 금, WTI, 비트코인 — WTI는 5번 순차 재검증으로 추가 확보).

**여전히 대체/보완 필요한 항목** (2개, 4개 세부 심볼):
- **지수 선물(S&P·나스닥)**: FMP 플랜 제한. AV에는 선물 관련 엔드포인트 자체가 없음. → 새 소스 필요(PlayMCP 등에서 재검토) 또는 타일에서 제외.
- **KOSPI·KOSDAQ**: FMP 플랜 제한, AV `INDEX_DATA`는 순차 호출로도 확인된 진짜 플랜 게이트(무료 키로 원천 불가). → 국내 지수 특화 소스(PlayMCP의 웹 스크래핑/브라우징 기능, 네이버·다음 금융, KRX 공공데이터포털 등) 필요.

**소스별 무료 한도 요약 및 배분 제안**:
| 소스 | 무료 한도 | 이번 점검에서 확보한 역할 |
|---|---|---|
| FMP | 심볼/엔드포인트별로 플랜 게이트가 갈림 (일간 호출수 한도는 미확인, 429는 안 봄) | 미국 3대 지수, 닛케이, VIX, 10년물 금리, 금(선물), 비트코인, 실적/경제 캘린더, RSI/SMA — **1차 소스로 사용** |
| Alpha Vantage | 25 requests/day, 1 request/sec(버스트) — **순차 호출 필수** | 원/달러, 금(현물, FMP와 교차검증용), **WTI**, RSI/SMA(FMP 실패 시 백업) — 하루 호출 예산을 브리핑 1회 발행 기준으로 8~10건 이내로 설계 권장(캘린더성 확인·재시도 여유분 포함) |
| PlayMCP | 미확인 (다음 세션에서 점검 필요) | 지수 선물, KOSPI/KOSDAQ 등 나머지 2개 항목 커버 후보 |
